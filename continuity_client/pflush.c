...
//static int global_cpu_speed_mhz = 0;  //欣欣：相比level去掉的
//static int global_write_latency_ns = 0;  //欣欣：相比level去掉的
...
//欣欣：level 里是 static inline int emulate_latency_ns(int ns)
...
/*
Function: pflush() 
        Flush a cache line with the address addr;来自baseline 1

void pflush(uint64_t *addr, int ns)
{
    if (global_write_latency_ns == 0) {
        return;
    }

    // Measure the latency of a clflush and add an additional delay to meet the write latency to NVM 
