...
#define RDMA_BUFFER_SIZE (KEY_LEN + VALUE_LEN + 5)
#define QP_DEPTH 16000
#define INIT_SIZE 160000
#define WORKLOAD_SIZE 160000
...
struct sub_thread {
    ...
    uint8_t thread_allkey[KEY_LEN*WORKLOAD_SIZE/20];
...
static char workload_type;
static char* workload_dic = "../../../index-microbench/workloads/";
static char workload_init_file[100];
static char workload_run_file[100];
...
void query(struct connection *conn, const uint8_t *key, const uint8_t read_and_update, uint32_t thread_id);
...
int main(int argc, char** argv) {
    ...
    if (argc != 5) {
        printError("usage: client <server-address> <server-port> <thread_num> <workload_type>.");
    }
    ...
    workload_type = argv[4][0];
    printInfo("workload type: %c", workload_type);
    snprintf(workload_init_file, 100, "%sload%c_unif_int.dat", workload_dic, workload_type);
    snprintf(workload_run_file, 100, "%stxns%c_unif_int.dat", workload_dic, workload_type);
...
void register_memory(struct connection *conn) {
    ...
    for (i = 0; i < thread_num; i++) {
        ...
        conn->thread_conn[i].thread_id = i;
        conn->thread_conn[i].read_and_update = 0;
    }
}
...
void load_run_workloads(struct connection *conn) {
    ...
    while (fgets(buf, 50, file) != NULL) {
        ...
        if (strncmp(buf, "INSERT", 6) == 0) {
            ...
            while(strlen(key) < KEY_LEN - 1) {
                strncat(key, buf+7, KEY_LEN - strlen(key) - 1);
            }
            key[KEY_LEN-1] = '\0';
    ...
    for (i = 0; i < thread_num; i++) {
        if (workload_type == 'd')
            arr[i] = (struct thread_val*)calloc(WORKLOAD_SIZE, sizeof(struct thread_val));
        else
    ...
    if (workload_type == 'd') {
        while (fgets(buf, 50, file) != NULL) {
            ...
            if (strncmp(buf, "INSERT", 6) == 0) {
                memcpy(key, buf + 7, KEY_LEN-1);
                while(strlen(key) < KEY_LEN - 1) {
                    strncat(key, key, KEY_LEN - strlen(key));
                }
                memcpy(arr[insert_num % thread_num][move[insert_num % thread_num]].key, key, KEY_LEN-1);
                arr[insert_num % thread_num][move[insert_num % thread_num]].key[KEY_LEN-1] = '\0';
                arr[insert_num % thread_num][move[insert_num % thread_num]].op = 0;
                if (strlen(threads[insert_num%thread_num].thread_allkey) != 0) strcat(threads[insert_num%thread_num].thread_allkey, key);
                else memcpy(threads[insert_num%thread_num].thread_allkey, key, KEY_LEN-1);
                move[insert_num % thread_num]++;
                insert_num++;
            }
                ...
                if (thread_num > 1) {
                    for (; k < thread_num; k++) {
                        char* ptr = strstr(threads[k].thread_allkey, key);
                        if (ptr != NULL) {
                            break;
                        }
                    }
                ...
                if (k >= thread_num) {
                    k = 0; int l;
                    for (l = 1; l < thread_num; l++) {
                        if (move[l] < move[k]) k = l;
                    }
                }
                memcpy(arr[k][move[k]].key, key, KEY_LEN-1);
                arr[k][move[k]].key[KEY_LEN-1] = '\0';
                arr[k][move[k]].op = 3;
                move[k]++;
            }
            operation_num++;
        ...
        memset(key, 0, KEY_LEN);
        while (strlen(key) != 0 || (operation_num != WORKLOAD_SIZE && fgets(buf, 50, file) != NULL)) {
            if (strlen(key) == 0) {
                if (buf[strlen(buf)-1] == '\n') buf[strlen(buf)-1] = '\0';
                memcpy(key, buf + 5, KEY_LEN-1);
            }
            memcpy(arr[operation_num % thread_num][move[operation_num % thread_num]].key, key, KEY_LEN-1);
            ...
            arr[operation_num % thread_num][move[operation_num % thread_num]].read_and_update = 0;
            if (operation_num != (WORKLOAD_SIZE-1) && fgets(buf, 50, file) != NULL) {
                if (strncmp(buf, "UPDATE", 6) == 0) {
                    arr[operation_num % thread_num][move[operation_num % thread_num]].read_and_update = 1;
                    memset(key, 0, KEY_LEN);
                }
                else {
                    memcpy(key, buf + 5, KEY_LEN-1);
        ...
        while (fgets(buf, 50, file) != NULL) {
            ...
            if (strncmp(buf, "INSERT", 6) == 0) {
                ...
                memcpy(key, buf + 7, KEY_LEN-1);
                while(strlen(key) < KEY_LEN - 1) {
                    strncat(key, key, KEY_LEN - strlen(key)-1);
                }
                memcpy(arr[operation_num % thread_num][move[operation_num % thread_num]].key, key, KEY_LEN-1);
                arr[operation_num % thread_num][move[operation_num % thread_num]].key[KEY_LEN-1] = '\0';
                ...
                memcpy(key, buf + 5, KEY_LEN-1);
                while(strlen(key) < KEY_LEN - 1) {
                    strncat(key, buf + 5, KEY_LEN - strlen(key) - 1);
                }
                memcpy(arr[operation_num % thread_num][move[operation_num % thread_num]].key, key, KEY_LEN-1);
                arr[operation_num % thread_num][move[operation_num % thread_num]].key[KEY_LEN-1] = '\0';
                arr[operation_num % thread_num][move[operation_num % thread_num]].op = 3;
...
void* thread_run(void* args) {
    ...
    for (i = 0; i < subthread->op_num; i++) {
        ...
        if (subthread->arr[i].op == 0) {        // insert
            ...
            a2++;
            query(subthread->conn, subthread->arr[i].key, subthread->arr[i].read_and_update, subthread->id);
        }
    }
}
...
void query(struct connection *conn, const uint8_t *key, const uint8_t read_and_update, uint32_t thread_id) {
   ...
   // conn->thread_conn[thread_id].start2_flag = 1;
    ...
    // 可以用一个操作代替吗？
    memcpy((void *)(conn->thread_conn[thread_id].hash_key), (void *)(key), KEY_LEN);
    conn->thread_conn[thread_id].read_and_update = read_and_update;
...
void read_remote(struct connection* conn, uint32_t thread_id, uint32_t len, uint64_t offset) {
    ...
    //wr.wr_id = (uintptr_t)((conn->thread_conn[thread_id].thread_id));
...
void write_with_imm(struct connection *conn, uint32_t thread_id) {
    //if(write_count >= INIT_SIZE) printInfo("%d write_with_imm start.\n", thread_id);
    struct ibv_send_wr wr, *bad_wr = NULL;
    struct ibv_sge sge;
...
void on_completion(struct ibv_wc *wc) {
    ...
    if (wc->opcode == IBV_WC_RDMA_WRITE) {
        ...
        //uint32_t thread_id = (uint32_t)wc->wr_id;
        //struct thread_connection *threadconn = (struct thread_connection *)&s_ctx->conn->thread_conn[thread_id];
        write_count++;
        ...
        if (write_count > INIT_SIZE && time_flag == 0) {
            if (threadconn->start_flag == 0) {
                ...
                //printInfo("write, latency: %lf, %lu %lu", single_time, threadconn->start.tv_sec, threadconn->start.tv_nsec);
            }
            threadconn->start_flag = 1 - threadconn->start_flag;
        }
        //printInfo("single time: %lf", single_time);
        //if (write_count > INIT_SIZE) printInfo("%d WRITE OPERATION OK.", threadconn->thread_id);
...
READEND:
        if (ret == 0) {
            printInfo("read: for key: %s, value not found. %d", threadconn->hash_key, strlen(threadconn->hash_key));
        }
        ...
        //printInfo("read, lantency: %lf", single_time);
        threadconn->start2_flag = 1 - threadconn->start2_flag;
        //threadconn->canRead = 1;
        if (threadconn->read_and_update == 1) {
            threadconn->start = threadconn->finish2;
            uint8_t val[VALUE_LEN];
            //pthread_mutex_unlock(&threadconn->read_mutex);
            memcpy(val, threadconn->hash_key, VALUE_LEN);
            val[VALUE_LEN-1] = '\0';
            //while (threadconn->canWrite == 0);
            //threadconn->canWrite = 0;
            pthread_mutex_lock(&threadconn->write_mutex);
            sprintf(threadconn->buffer, "1 %s %s", threadconn->hash_key, val);      // use 1 to signal update operation
            write_with_imm(s_ctx->conn, threadconn->thread_id);
        }
        pthread_mutex_unlock(&threadconn->read_mutex);
    }
